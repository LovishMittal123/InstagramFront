...
const Signup = () => {
  ...
  const [previewSrc, setPreviewSrc] = useState(""); // preview URL
  ...
  const signup = async (e) => {
    ...
    try {
      ...
      if (photoFile) form.append("photo", photoFile); // append photo if selected
      ...
      >
        ...
        <div className="mt-4 flex flex-col md:flex-row items-center gap-3">
          <button
            type="submit"
            disabled={loading}
            className={`flex-1 inline-flex items-center justify-center gap-2 py-2 rounded-lg text-white ${
              loading ? "bg-sky-400 cursor-wait" : "bg-sky-600 hover:bg-sky-700"
            }`}
          >
            {loading ? (
              <svg
